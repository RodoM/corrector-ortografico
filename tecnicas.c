...
/**
 * Chequea de que la sugerencia no sea repetida, en caso de no serlo y que no
 * sea el ultimo paso, la agrega a la tabla de cambios que se usara en el
 * siguiente paso, si ademas pertenece al diccionario y no fue previamente
 * agregada se agrega a la lista de sugerencias validas.
 */
void validar_sugerencia(char *sugerencia, TablaHash diccionario,
...
void tecnica_intercambiar(TablaHash diccionario, TablaHash cambiosActuales,
  ...
  for (unsigned int i = 0;
       i < cambiosActuales->capacidad && *cantSugerencias < 5; i++) {
    if (cambiosActuales->elems[i].dato != NULL) {
      ...
      for (int j = 0; j < lenSugerencia - 1 && *cantSugerencias < 5; j++) {
        if (sugerencia[j] != sugerencia[j + 1]) {
          ...
          validar_sugerencia(sugerencia, diccionario, cambiosNuevos,
      ...
      if (cambiosActuales->elems[i].lista != NULL) {
        for (SNodo * nodo = cambiosActuales->elems[i].lista;
             nodo != NULL && *cantSugerencias < 5; nodo = nodo->sig) {
  ...
  for (unsigned int i = 0;
    ...
    if (cambiosActuales->elems[i].dato != NULL) {
      ...
      for (int j = 0; j < lenSugerencia && *cantSugerencias < 5; j++) {
        ...
        validar_sugerencia(sugerencia, diccionario, cambiosNuevos, sugerencias,
  ...
  for (unsigned int i = 0;
    ...
    if (cambiosActuales->elems[i].dato != NULL) {
      ...
      for (int j = 1; j < lenSugerencia && *cantSugerencias < 5; j++) {
        ...
        if (tablahash_contiene(diccionario, sugerencia)
            && tablahash_contiene(diccionario, sugerencia2)) {
          sugerencia[j] = ' ';
          sugerencia[j + 1] = '\0';
